...
function sortTable(index, type, isSorted) {
  ...
  function getColumnValue(row, cellIndex) {
    if (type === "integer" || type === "double") {
      ...
      const dateA = row.cells[cellIndex].innerText
        .split(".")
        .reverse()
        .join("-");
  ...
  function quickSort(arr, columnIndex) {
    ...
    let pivorRow = arr[0];
    let pivot = getColumnValue(pivorRow, columnIndex);
    const left = [];
    const right = [];
    ...
    return quickSort(left, columnIndex).concat(pivorRow, quickSort(right, columnIndex));
  }
